using LaboratoryTen.Engine;

namespace LaboratoryTen.Ui
{
    public class ConsoleUi
    {
        ...
        public static void Run()
        {
            ...
            try
            {
                int n = GetNumberFromConsole("количество строк N");
                int m = GetNumberFromConsole("количество столбцов M");
                ...
                double[,] matrix = GetMatrixByCounts(n, m);
                int[] result = MatrixOpiration.CountOfElementsGreaterAverageArefmetric(matrix);
        ...
        private static double[,] GetMatrixByCounts(int n, int m)
        {
            double[,] array = new double[n, m];

            for (int indexOfLine = 0; indexOfLine < n; indexOfLine++)
            {
                for (int indexOfColumn = 0; indexOfColumn < m; indexOfColumn++)
                {
